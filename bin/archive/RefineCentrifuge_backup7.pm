...
sub refine_centrifuge_results{
    ...
    my $read_cutoff=10; # discard hits with less than 10 reads
    my $cap_size = 2000000;
    my ($centrifuge_hit_info, $centrifuge_read_info, $lineage, $genome_info, $host_reads) = get_centrifuge_hits("$output_dir/$hit_table", $seq_type, $nodes, $names, $phylogeny, $acc2genome_file, $hit_length_cutoff, $cap_size*2.5); # use genome_assembly file as the key
    my $centrifuge_hit_size = get_hit_size($centrifuge_hit_info);
    my ($centrifuge_hier_hit_info, $centrifuge_hit_fate) = centrifuge_hier_clustering($centrifuge_hit_info, $centrifuge_read_info, $read_cutoff);
	...
	update_hits($centrifuge_hier_hit_info, \%unmapped_reads, $read_cutoff, $top_cent_hits);
	%unmapped_reads=();
...
sub get_centrifuge_hits{
	...
	$counter++;
	if (scalar(keys%records) == $cap_size) {
	    last;
	}
