...
sub refine_centrifuge_results{
    ...
    my ($hit_info, $read_info, $unique_hit_info, $lineage) = get_centrifuge_hits("$output_dir/$hit_table", $seq_type, $nodes, $names, $phylogeny); # use acc_no as the key
    ...
    my $genome_size = prepare_ref_genome($output_dir, $unique_hit_info, $acc2genome_file, $reads_cutoff);
    ...
    my ($hier_hit_info, $hier_read_info, $hit_fate) = filter_reassign_reads($bwa_read_info, $bwa_hit_info, $reads_cutoff);
...
sub get_centrifuge_hits{
	...
	if (scalar(@hits_list)<=3) {
		...
		$unique_hit_info{$j}{'taxid'} = $hit_info{$j}{'taxid'};
	    }

	}
	
    }
    return (\%hit_info, \%read_info, \%unique_hit_info, \%lineage);
}
...
sub prepare_ref_genome{
    ...
    my %genome_size;
    my %genome_info;
    my $ref_genome_read_count_cutoff = shift;
...
sub filter_reassign_reads{
	...
	foreach my $x (keys%{$hit_info}){
		...
		delete($hit_info->{$x});
		$hit_fate->{$x} = 'read_number_below_cutoff';
	    }
