...
sub refine_centrifuge_results{
    ...
    my ($hit_info, $read_info, $lineage) = get_hits("$output_dir/$hit_table", $seq_type, $nodes, $names, $phylogeny); # use acc_no as the key
...
sub sam2cov{
    ...
    system("Rscript $script_dir/bin/read_depth_plot.r -l $taxid -n $strain -r $read_size -b $dir/genom_cov/$taxid.sorted.bam -s $geno_size -o $dir/genom_cov/$taxid\_cov.pdf >/dev/null 2>&1");
...
sub prepare_ref_genome{
	...
	system("aws s3 cp s3://zymo-files/WGS_Pipeline/shotgun_database/20200327_genomes/$genome_id $dir/ref_genomes/$taxid.fasta.gz >/dev/null 2>&1");
