...
my $R1 = "$input_dir/in732_1_R1.fastq";
my $R2 = "$input_dir/in732_1_R2.fastq";
my $seq_type = "illumina.pe";
my $sample_id = 'in732_1';
my $scirpt_dir = '/home/stdell/Desktop/shotgun_pipeline/ubuntu/scripts';

my ($taxa2abun, $hits_info, $hit_fate) = refine_centrifuge_results("$output_dir", "in732_1_class.tsv", $nodes, $names, $phylogeny, $seq_type, $R1, $R2, $database, $threads, $sample_id, $script_dir);
...
sub refine_centrifuge_results{
    ...
    my ($centrifuge_hit_info, $centrifuge_read_info, $lineage, $genome_info, $host_reads) = get_centrifuge_hits("$output_dir/$hit_table", $seq_type, $nodes, $names, $phylogeny, $acc2genome_file, $hit_length_cutoff); # use genome_assembly file as the key
    ...
    my $subset_size = 100000; # the number of reads to be subset 
	...
	bwa_mapping($R1, $R2, $output_dir, $threads, $hit_length_cutoff);
    }
...
sub get_centrifuge_hits{
    ...
    return (\%hit_info, \%read_info, \%lineage, \%genome_info, \%host_reads);
}
