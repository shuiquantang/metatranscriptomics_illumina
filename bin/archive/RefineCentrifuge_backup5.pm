...
sub TaxaPolisher{
	...
	my $taxa2abun = refine_centrifuge_results("$output_dir", "$i\_class.tsv", $nodes, $names, $phylogeny, $seq_type, $R1, $R2, $database, $threads_per_samples, $i, $script_dir);
...
sub refine_centrifuge_results{
    ...
    my ($centrifuge_hit_info, $centrifuge_read_info, $lineage, $genome_info) = get_centrifuge_hits("$output_dir/$hit_table", $seq_type, $nodes, $names, $phylogeny, $acc2genome_file, $hit_length_cutoff); # use genome_assembly file as the key
    my $centrifuge_hit_size = get_hit_size($centrifuge_hit_info);
    system("echo step2 start: hier centrifuge results\n");
    my $reads_cutoff=10; # if ignore hits that have less than 10 reads
    ...
    %{$centrifuge_hit_info}=();
    my $centrifuge_unique;
    my $centrifuge_hier_hit_size = get_hit_size($centrifuge_hier_hit_info);
    %{$centrifuge_hier_hit_info}=();#empty to release ram
    my @centrifuge_hits = sort{$centrifuge_hier_hit_size->{$b}<=>$centrifuge_hier_hit_size->{$a}}keys%{$centrifuge_hier_hit_size};
    my $cent_hit_size = scalar(@centrifuge_hits);
    ...
    my $cap_size = 5000000;
    my $ave_read_size = simplify_fastq_header($output_dir, $new_R1, $new_R2, $cap_size);
    my $bk_R2 = 'R2.bk.fastq';
    my $bk_R1 = 'R1.bk.fastq';
    system("cp $output_dir/$new_R1 $output_dir/$bk_R1");
    system("cp $output_dir/$new_R2 $output_dir/$bk_R2");
    my %bwa_hier_hit_size;
    my %unmapped_reads;
    my $step_wise_hit_num = 100; # the number of candidate hits to analyze in each interation
    my $subset_size = 500000; # the number of reads to be subset 
    for (my $i=0;$i<$cent_hit_size;$i+=$step_wise_hit_num){
	%unmapped_reads = ();
	my @query;
	my $upper = $i+$step_wise_hit_num-1;
	if ( $upper <= $cent_hit_size-1) {
	    @query = @centrifuge_hits[$i..$upper];
	}else{
	    @query = @centrifuge_hits[$i..$cent_hit_size-1];
	}
	download_genomes($output_dir, \@query, $genome_info);# download and concatenate contigs
	format_ref_db(\@query, $output_dir);
	bwa_mapping($new_R1, $new_R2, $output_dir, $threads, $hit_length_cutoff);
	...
	my $survived_hit_size = bwa_hier_clustering($hit_info, $read_info, $reads_cutoff); # parse the sam file again, reduce the abundance of a hit by 1 if a assigned read was reassigned
	my $left_hit_size=();
	if ($reads == $subset_size) {
	    ...
	    $left_hit_size = bwa_hier_clustering($hit_info, $read_info, $reads_cutoff);
	}
	save_hits(\%bwa_hier_hit_size, $survived_hit_size, $left_hit_size);
	update_reads($output_dir, $bk_R1, $bk_R2, $new_R1, $new_R2, \%unmapped_reads);
    }

    
    my $unmapped_read_no = scalar(keys%unmapped_reads);
    my @survival =  keys%bwa_hier_hit_size;
    if ($cent_hit_size > $step_wise_hit_num) {
	format_ref_db(\@survival, $output_dir);
	bwa_mapping($bk_R1, $bk_R2, $output_dir, $threads, $hit_length_cutoff);
    }
    ...
    my $opt_hit_size = filter_hits_by_genome_coverage($NR_hit_size, $output_dir, $genome_info, $ave_read_size, $lineage, $script_dir);
...
sub bwa_mapping{
    ...
    system("bwa mem -a -P -t $threads $dir/ref_genomes/ref.fasta $dir/$R1 $dir/$R2 > $dir/results1.sam");
...
sub simplify_fastq_header{
    ...
    my $ave_read_size = $bp/$counter/2;
    return ($ave_read_size);
}
