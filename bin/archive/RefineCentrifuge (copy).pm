...
sub TaxaPolisher{
    ...
    $threads_per_samples = int($cpu/$threads);
	...
	if ($seq_type =~ /\.pe/) {
	    $sample_type = 2;
	    $R1 = "$input_dir/$i\_R1.fastq";
	    $R2 = "$input_dir/$i\_R2.fastq";
	}else{
	    $sample_type = 1;
	...
	}elsif(-z "$output_dir/$i\_class.tsv"){ # no reads mapped to reference genome
	    my %taxa2abun=();
	    $sample2taxa_abun_table{$i}=\%taxa2abun;
	}else{
	    my $taxa2abun = refine_centrifuge_results("$output_dir", "$i\_class.tsv", $nodes, $names, $phylogeny, $seq_type, $R1, $R2, $database, $threads_per_samples, $i, $script_dir, $cap_size);
	    $sample2taxa_abun_table{$i}=$taxa2abun;
	    cleanup_temp_files($input_dir, $output_dir, $i);
	}
    ...
    summarize_read_distribution($input_dir, $output_dir, \@samples, $cap_size, $abun_table_folder);
}


sub summarize_read_distribution{
    my $trim_dir = shift;
    my $cent_dir = shift;
    my $samples = shift;
    my $cap_size = shift;
    ...
    my %read_info;
    open(my $f1, "<$trim_dir/summary.tsv") or die;
    my $header = <$f1>;
    while (my $line = <$f1>) {
	chomp $line;
	my @coln = split(/\t/, $line);
	$read_info{$coln[0]}{'total'}=$coln[7];
	$coln[3] = substr($coln[3], 0, length($coln[3])-1);# remove '%' in the end
	$read_info{$coln[0]}{'perc_after_trim'}=$coln[3];
    }
    close $f1;
    foreach my $i (@{$samples}){
	if (-e "$cent_dir/$i/host_reads.txt") {
	    open(my $f2, "<$cent_dir/$i/host_reads.txt") or die;
	    while (my $line = <$f2>) {
	        if ($line =~ /^>/) {
	            chomp $line;
	            my @coln = split(/\ /, $line);
	            $read_info{$i}{'host'}=$coln[1];
	        }
	    }
	    close $f2;
	}else{
	...
	if (-e "$cent_dir/$i/R1.fastq") {
	    my $unmapped_reads = `cat $cent_dir/$i/R1.fastq | wc -l`;
	    chomp $unmapped_reads;
	    $read_info{$i}{'unmapped'}=$unmapped_reads/4;
	    system("rm $cent_dir/$i/R1.fastq $cent_dir/$i/R2.fastq");
	}else{
	    $read_info{$i}{'unmapped'}=0;
	}
	
    }
    open(my $f4, ">$abun_table_dir/host_dna_abun.csv") or die;
    my @header = @{$samples};
    unshift(@header, '#taxon_id');
    $header = join("\t", @header);
    my @host = ('Host_reads');
    my @microbe = ('Microbial_reads');
    my @unclassified = ('Unclassified');
    foreach my $i (@{$samples}){
	my $tot_reads = $read_info{$i}{'total'};
	my $perc_after_trim = $read_info{$i}{'perc_after_trim'};
	my $host_reads = $read_info{$i}{'host'};
	my $non_host_reads = $tot_reads * $perc_after_trim/100 - $host_reads;
	my $unmapped = $read_info{$i}{'unmapped'};

	if ($non_host_reads > $cap_size) {
	    $unmapped = int($unmapped/$cap_size*$non_host_reads);
	    
	}
	
	my $microbial_reads=$non_host_reads-$unmapped;
	push(@host, $host_reads);
	push(@microbe, $microbial_reads);
	push(@unclassified, $unmapped);
    }
    my $host_str = join("\t", @host);
    my $microbe_str = join("\t", @microbe);
    my $unclassified_str = join("\t", @unclassified);
    print($f4 "$header\n$host_str\n$microbe_str\n$unclassified_str\n");
    close $f4;
}
...
sub cleanup_temp_files{
    ...
    my $dir = shift;
    my $sample_id = shift;
    #system("rm -r $dir/$sample_id/genom_cov/*.bam $dir/$sample_id/genom_cov/*.bai $dir/$sample_id/genom_cov/*.txt");
    ...
    system("rm -r $dir/$sample_id\_report.tsv");
    system("rm -r $dir/$sample_id\_class.tsv");
    system("rm -r $trim_dir/$sample_id\_*.fastq");
}
...
sub refine_centrifuge_results{
    ...
    my $read_cutoff=$cap_size*0.00005;
    if ($read_cutoff<10) {
	$read_cutoff=10;
    }
	...
	download_genomes($output_dir, $top_cent_hits, $genome_info, $database);# download and concatenate contigs
...
sub download_genomes{
	...
	system("cp $db_dir/genomes/$genome_file $dir/ref_genomes/$assembly_id.fasta.gz");
	...
	close $f2;
	close $f3;
	if ($size < 200000000) {# remove big eukaryote genomes
...
sub get_centrifuge_hits{
	...
	#    next;
	#}
	my @taxid_to_exclude = (9606,9605,9604,9443,10090,10080,10066,9989,10116,10114, 9615,9611,9608,33554,9685,9682,9681,40674,7711,33208);
	# 9606 homo sapien, 9605 homo, 9604, hominidae, 9443 primates, 10090 Mus musculus, 10116 Rattus norvegicus, 9615 Canis lupus familiaris, 9685 Felis catus, 40674 Mammalia,7711 phylum Chordata, 33208 Metazoa kingdom
	my $exclude = 0;
	foreach my $m (@taxid_to_exclude){
	    if ($taxid eq $m) {
		$exclude=1;
	...
	if ($acc_no eq 'unclassified') {#don't add them into the list the list of above strain, one read can be unclassified in database1 and assigned in database 2.
	    next;
	}
	my $read_id = $coln[0];
	if ($acc_no !~ /\.|^zymo/) {
    ...
    my %assembly_to_exclude = (
	    ...
	    'GCA_006568855.1' => 3, #human dna comtaminations
	    'GCA_009025735.1' => 3,
	    'GCA_003057255.1' => 3,
	    'GCA_004522515.1' => 3,
	    'GCA_900093885.1' => 3,
	...
	if (exists($host_reads{$read_id})) { # exclude the reads that has previous been assigned to the host as some reads can have multiple assignments (because of reference genome contamination)
	    next;
	}
...
sub filter_hits_by_genome_coverage{
	...
	if (removed_by_genome_cov($dir, $cov_table, $ref_genome, $tot_mapped_reads, $genome_info, $assemblyid, $read_size)) {
	}else{
...
sub sam2cov{
    ...
    system("samtools view -S -b -F 256 $sam_file >$dir/genom_cov/$assemblyid.bam");# only keep the primary alignments
...
sub removed_by_genome_cov{
    ...
    my $no_fragments = int($genome_info->{$assemblyid}->{'size'}/$unit_size);
    ...
    my $accu_reads = 0;
    for (my $i=0; $i<=0.2*$maximum_fragments;$i++){ #10% regions of maximum possible regions
